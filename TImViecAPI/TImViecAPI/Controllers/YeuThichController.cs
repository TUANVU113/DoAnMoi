...
namespace TImViecAPI.Controllers
{
    ...
    public class YeuThichController : ControllerBase
    {
        ...
        [HttpGet("check/{tinId}")]
        [Authorize(Roles = "UngVien")]
        public async Task<IActionResult> CheckYeuThich(int tinId)
        {
            string? username = User.Identity?.Name;
            if (string.IsNullOrEmpty(username))
                return Unauthorized("Vui lòng đăng nhập.");

            var user = await _context.NguoiDung.FirstOrDefaultAsync(nd => nd.tkName == username);
            if (user == null)
                return Unauthorized("Không tìm thấy ứng viên.");

            bool daLuu = await _context.CongViecYeuThich
                .AnyAsync(x => x.ungvienID == user.tkid && x.tintuyenID == tinId);

            return Ok(new { isFavorite = daLuu });
        }
